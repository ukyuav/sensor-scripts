...
a document conversion deck program, which we will use to create the PDF's.
Finally you will need to install a LaTeX template, any template that supports
code listings will work. I recommend using the [[https://github.com/Wandmalfarbe/pandoc-latex-template][Eisvogel]] template, specifically
the [[https://github.com/jlacko/pandoc-latex-template][modfied version]] that adds highlights to hyperlinks.
...
what we would like to use code listings, which gives us code blocks with syntax
